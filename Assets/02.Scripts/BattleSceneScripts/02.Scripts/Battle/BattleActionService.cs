...
public class BattleActionService
{
    ...
    private readonly CombatProcessor _combatProcessor = new CombatProcessor();
    ...
    public void TryPlayerAttack(Direction rawDir, Action onComboAvailable, Action onTurnFinished)
    {
        ...
        Direction filteredDir = _runtime.Player.statusHandler.FilterDirection(rawDir);
        if (filteredDir == Direction.None) return;

        _skillExecutor.OnDirectionInput(filteredDir);
        _runtime.Enemy.statusHandler.OnDirectionInput();
    ...
    private IEnumerator PlayerAttackSequence(Direction playerDir, Action onComboAvailable, Action onTurnFinished)
    {
        ...
        if (result.success)
        {
            int damage = _combatProcessor.CalculateFinalDamage(_runtime.Player, _runtime.Enemy, wasCombo);
            _runtime.Enemy.TakeDamage(damage);
            CanUseSkill = true;
    ...
    private IEnumerator EnemyAttackSequence(Direction enemyDir, Direction playerDir, int turnCount, Action onEnemyTurnResolved)
    {
        ...
        if (result.success)
        {
            _runtime.Player.TakeDamage(_runtime.Enemy.statData.attackDamage);
            Debug.Log($"<color=red>Defense failed!</color> Took {_runtime.Enemy.statData.attackDamage} damage.");
        }
        else
        {
